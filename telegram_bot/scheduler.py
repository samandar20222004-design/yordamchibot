...
async def _send_by_type(bot, target_chat, post_type, content, file_id, markup):
    caption = content or ""
    if post_type == "text":
        await _send_with_fallback(bot.send_message, chat_id=target_chat, text=caption, reply_markup=markup)
    elif post_type == "photo":
        await _send_with_fallback(bot.send_photo, chat_id=target_chat, photo=file_id, caption=caption, reply_markup=markup)
    elif post_type == "video":
        await _send_with_fallback(bot.send_video, chat_id=target_chat, video=file_id, caption=caption, reply_markup=markup)
    elif post_type == "animation":
        await _send_with_fallback(bot.send_animation, chat_id=target_chat, animation=file_id, caption=caption, reply_markup=markup)
    elif post_type == "document":
        await _send_with_fallback(bot.send_document, chat_id=target_chat, document=file_id, caption=caption, reply_markup=markup)
    elif post_type == "audio":
        await _send_with_fallback(bot.send_audio, chat_id=target_chat, audio=file_id, caption=caption, reply_markup=markup)
    elif post_type == "voice":
        await _send_with_fallback(bot.send_voice, chat_id=target_chat, voice=file_id, caption=caption, reply_markup=markup)
