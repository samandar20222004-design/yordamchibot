...
async def start_add_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await update.message.reply_text(
      ...
      "2. Kanal/guruhdan biror xabarni bu yerga <b>Forward</b> qiling yoki"
      " <code>@username</code>ini yozing.",
...
async def channel_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if identifier is None:
    await msg.reply_text(
        "⚠️ Kanal aniqlanmadi. Xabarni Forward qiling yoki @username yuboring:"
    )
    return ADD_CHANNEL
  ...
  try:
    ...
    await msg.reply_text(
        "⚠️ Kanal topilmadi yoki bot u yerda admin emas.",
        reply_markup=get_cancel_keyboard(),
    )
    return ADD_CHANNEL
  ...
  if db.save_channel(user_id, channel_id, channel_title):
    ...
    await msg.reply_text(
        "❌ Saqlashda xatolik.", reply_markup=get_main_keyboard(is_admin)
    )
  return ConversationHandler.END
...
async def on_bot_chat_member_update(
  ...
  if new_status in ("administrator", "creator", "member"):
    if chat.type == "channel" and new_status not in ("administrator", "creator"):
      return
    ok = db.save_channel(
        adder.id if adder else 0, str(chat.id), chat.title or "Nomsiz"
    )
    if ok and adder:
      try:
        await context.bot.send_message(
            chat_id=adder.id,
            text=(
                f"✅ <b>{html_escape(chat.title)}</b> avtomatik ulandi! Endi"
                " post rejalashtirishingiz mumkin."
            ),
            parse_mode="HTML",
        )
      except Exception:
