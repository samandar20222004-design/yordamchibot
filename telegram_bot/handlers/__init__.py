...
from handlers.admin import (
    ...
    broadcast_send,
    broadcast_start,
    del_sponsor_callback,
    show_statistics,
    sponsor_channel_received,
    sponsors_menu,
    start_add_sponsor,
    start_set_ad,
    ad_text_received,
...
from handlers.pending import (
    EDIT_POST_TIME,
    cancel_post_callback,
    edit_post_time_received,
    ...
    for row in query.message.reply_markup.inline_keyboard:
      ...
      for btn in row:
        if btn.callback_data and btn.callback_data.startswith("react:"):
          ...
          new_row.append(
              btn.__class__(new_label, callback_data=btn.callback_data)
          )
        else:
          new_row.append(btn)
      keyboard.append(new_row)
    await query.edit_message_reply_markup(
        reply_markup=query.message.reply_markup.__class__(keyboard)
    )
...
def register_all_handlers(app):
  global_jump_handlers = [
      ...
      MessageHandler(
          exact(BTN_CHANNELS), lambda u, c: _jump_to(u, c, channels_menu)
      ),
      MessageHandler(
          exact(BTN_PENDING), lambda u, c: _jump_to(u, c, list_pending_posts)
      ),
      MessageHandler(
          exact(BTN_PROFILE), lambda u, c: _jump_to(u, c, user_profile)
      ),
      MessageHandler(
          exact(BTN_ADMIN_PANEL), lambda u, c: _jump_to(u, c, admin_panel_menu)
      ),
      MessageHandler(
          exact(BTN_STATS), lambda u, c: _jump_to(u, c, show_statistics)
      ),
      MessageHandler(
          exact(BTN_ALL_POSTS), lambda u, c: _jump_to(u, c, admin_all_posts)
      ),
      MessageHandler(
          exact(BTN_ALL_CHANNELS),
          lambda u, c: _jump_to(u, c, admin_all_channels),
      ),
      MessageHandler(exact(BTN_BROADCAST), broadcast_start),
      MessageHandler(
          exact(BTN_SPONSORS), lambda u, c: _jump_to(u, c, sponsors_menu)
      ),
  ...
  main_conv = ConversationHandler(
      ...
      states={
          ...
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, channel_chosen
              )
          ],
          GET_CONTENT: global_jump_handlers
          + [
              MessageHandler(
                  filters.ALL & ~filters.COMMAND, content_received
              )
          ],
          GET_BTN_TITLE: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, btn_title_received
              )
          ],
          GET_BTN_URL: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, btn_url_received
              )
          ],
          GET_REACTIONS: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, reactions_received
              )
          ],
          GET_AUTO_DELETE: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, auto_delete_received
              )
          ],
          GET_TIME: global_jump_handlers
          + [MessageHandler(filters.TEXT & ~filters.COMMAND, time_received)],
          DAILY_TIME: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, daily_time_received
              )
          ],
          RECUR_DAY: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, recur_day_chosen
              )
          ],
          RECUR_TIME: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, recur_time_received
              )
          ],
          GET_DURATION: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, duration_chosen
              )
          ],
          ADD_CHANNEL: global_jump_handlers
          + [
              MessageHandler(
                  filters.ALL & ~filters.COMMAND, channel_received
              )
          ],
          ...
          ADD_SPONSOR_CHANNEL: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, sponsor_channel_received
              )
          ],
          SET_AD_TEXT: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, ad_text_received
              )
          ],
          EDIT_POST_TIME: global_jump_handlers
          + [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, edit_post_time_received
              )
          ],
      },
      fallbacks=[
  ...
  app.add_handler(
      CallbackQueryHandler(
          subscription_check_callback, pattern=r"^check_subscription$"
      )
  )
  app.add_handler(
      CallbackQueryHandler(del_sponsor_callback, pattern=r"^del_sponsor:")
  )
  app.add_handler(CallbackQueryHandler(reaction_callback, pattern=r"^react:"))
  app.add_handler(
      CallbackQueryHandler(cancel_post_callback, pattern=r"^cancel_post:")
  )
  app.add_handler(
      CallbackQueryHandler(remove_channel_callback, pattern=r"^remove_channel:")
  )
  app.add_handler(
      ChatMemberHandler(
          on_bot_chat_member_update, ChatMemberHandler.MY_CHAT_MEMBER
      )
  )
