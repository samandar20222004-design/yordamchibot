...
from keyboards.default import (
    ...
    WEEKDAY_LABELS,
    WEEKDAY_MAP,
    get_auto_delete_keyboard,
    get_button_prompt_keyboard,
    get_cancel_keyboard,
...
async def start_new_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if not channels:
    await update.message.reply_text(
        "⚠️ Ulangan kanal yoki guruh topilmadi!\n\nAvval '📢 Kanal/Guruhlar'"
        " bo'limidan kanal yoki guruhingizni ulang.",
...
async def channel_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if text == BTN_ALL_CHANNELS_TARGET:
    ...
    if text not in channels_map:
      await update.message.reply_text(
          "⚠️ Bunday kanal topilmadi. Qaytadan tanlang:"
      )
      return CHOOSE_CHANNEL
  ...
  await update.message.reply_text(
      ...
      " uchun kontentni yuboring (Matn, rasm, video, audio yoki boshqa kanaldan"
...
async def btn_title_received(
  ...
  await update.message.reply_text(
      f"🔗 '{text}' tugmasi bosilganda ochiladigan havola yoki kanal"
      " username'ini yuboring:\n\nMasalan: @kanalim yoki https://sayt.uz",
      reply_markup=get_cancel_keyboard(),
  )
  return GET_BTN_URL
...
async def btn_url_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if btn_link.startswith("@"):
    btn_link = f"https://t.me/{btn_link.replace('@', '')}"
      ...
      btn_link.startswith("http://")
      or btn_link.startswith("https://")
      or btn_link.startswith("t.me/")
  ):
    if "." in btn_link:
...
async def reactions_received(
  ...
  await update.message.reply_text(
      ...
      " bot uni kanaldan avtomatik o'chirib tashlaydi:",
...
async def auto_delete_received(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
  text = update.message.text.strip()
  hours = 0
...
async def _save_and_finish(
  ...
  channels = (
      ...
      else [(selected_channel_id, context.user_data.get("selected_channel_title"))]
  )
  ok_count = 0
  ...
  for ch_id, _ in channels:
    pid = db.add_post(
        ...
        content=content,
        file_id=file_id,
        scheduled_time=post_time_tz,
        recurrence_type=recurrence_type,
        recurrence_day=recurrence_day,
        recurrence_time=recurrence_time_str,
        end_date=end_date,
        btn_text=btn_text,
        btn_url=btn_url,
  ...
  if ok_count:
    if recurrence_type == "daily":
      ...
      when_text = (
          f"📅 Har {WEEKDAY_LABELS.get(recurrence_day)}, soat"
          f" {recurrence_time_str[:5]} da"
      )
    else:
      when_text = f"⏰ {post_time_tz.strftime('%Y-%m-%d %H:%M')}"

    del_info = (
        f"\n⏳ Kanalda turish muddati: {delete_after_hours} soat"
        if delete_after_hours > 0
        else ""
    )
    await update.message.reply_text(
        f"✅ Post muvaffaqiyatli rejalashtirildi!\n\n📢 Joylash:"
        f" {context.user_data['selected_channel_title']}\n{when_text}{del_info}",
        reply_markup=get_main_keyboard(is_admin),
    )
  else:
    await update.message.reply_text(
...
async def time_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if text == BTN_T_DAILY:
    await update.message.reply_text(
        "🔁 Har kuni soat nechida chiqsin?\nMasalan: 10:00 yoki 18:30",
        reply_markup=get_cancel_keyboard(),
    )
    return DAILY_TIME
  elif text == BTN_T_WEEKLY:
    await update.message.reply_text(
        "📅 Haftaning qaysi kuni chiqsin?",
  ...
  try:
    ...
    await update.message.reply_text(
        "⚠️ Format xato! Masalan: 2026-08-28 18:00 shaklida yuboring."
    )
    return GET_TIME
...
async def daily_time_received(
  ...
  await update.message.reply_text(
      "⏳ Post qancha muddat davomida har kuni chiqsin?",
...
async def recur_time_received(
  ...
  await update.message.reply_text(
      "⏳ Ushbu post qancha muddat davomida chiqsin?",
      reply_markup=get_duration_keyboard(),
  )
  return GET_DURATION
