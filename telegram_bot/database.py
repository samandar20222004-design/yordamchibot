...
def init_db():
  with db_cursor(commit=True) as cur:
    ...
    migrations = [
        ...
        " inline_button_text VARCHAR(255);",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS inline_button_url"
        " TEXT;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS enable_reactions"
        " BOOLEAN DEFAULT FALSE;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS delete_after_hours"
        " INTEGER DEFAULT 0;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS sent_message_id"
        " BIGINT;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS user_post_number"
        " INTEGER;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS recurrence_type"
        " VARCHAR(20) DEFAULT 'none';",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS recurrence_day"
        " INTEGER;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS recurrence_time"
        " TIME;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS end_date"
        " TIMESTAMP WITH TIME ZONE;",
        "ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS created_at"
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_scheduled_posts_status_time ON"
        " scheduled_posts (status, scheduled_time);"
    )
...
def add_sponsor_channel(
    channel_id: str, channel_title: str, channel_url: str
) -> bool:
  try:
    with db_cursor(commit=True) as cur:
      cur.execute(
...
def get_active_sponsors() -> list:
  try:
    with db_cursor() as cur:
      cur.execute(
          "SELECT id, channel_id, channel_title, channel_url FROM"
          " sponsor_channels WHERE is_active = TRUE ORDER BY id ASC"
      )
      return cur.fetchall()
  except Exception as e:
...
def remove_sponsor_channel(sponsor_id: int) -> bool:
  try:
    with db_cursor(commit=True) as cur:
      cur.execute(
          "DELETE FROM sponsor_channels WHERE id = %s", (sponsor_id,)
      )
...
def toggle_reaction(post_id: int, user_id: int, reaction: str) -> dict:
  try:
    with db_cursor(commit=True) as cur:
      cur.execute(
          "SELECT reaction_type FROM post_reactions WHERE post_id = %s AND"
          " user_id = %s",
          (post_id, user_id),
      )
      row = cur.fetchone()
      if row:
        if row[0] == reaction:
          ...
          cur.execute(
              "UPDATE post_reactions SET reaction_type = %s WHERE post_id = %s"
              " AND user_id = %s",
...
def _generate_user_code(cur) -> str:
  ...
  for _ in range(50):
    code = "".join(random.choice(letters) for _ in range(2)) + random.choice(
        string.digits
    )
    cur.execute("SELECT 1 FROM users WHERE user_code = %s", (code,))
    if not cur.fetchone():
      return code
  return "".join(random.choice(letters) for _ in range(3)) + "".join(
      random.choice(string.digits) for _ in range(2)
  )


def save_user(
    user_id: int, username: str, full_name: str = "", referrer_id: int = None
) -> bool:
  try:
    with db_cursor(commit=True) as cur:
...
def get_referral_stats(user_id: int) -> dict:
  try:
    with db_cursor() as cur:
      ...
      return {"referrals_count": cur.fetchone()[0]}
  except Exception as e:
...
def get_user_code(user_id: int) -> str:
  try:
    with db_cursor() as cur:
      cur.execute(
          "SELECT user_code FROM users WHERE user_id = %s", (user_id,)
      )
      row = cur.fetchone()
...
def get_user_channels(user_id: int) -> list:
  try:
    with db_cursor() as cur:
      cur.execute(
          "SELECT channel_id, channel_title FROM channels WHERE user_id = %s"
          " AND is_active = TRUE ORDER BY id ASC",
          (user_id,),
      )
      return cur.fetchall()
  except Exception as e:
...
def remove_channel(
    user_id: int, channel_id: str, is_admin: bool = False
) -> bool:
  try:
    with db_cursor(commit=True) as cur:
      if is_admin:
        ...
        cur.execute(
            "UPDATE channels SET is_active = FALSE WHERE channel_id = %s AND"
            " user_id = %s",
...
def add_post(
  ...
  try:
    with db_cursor(commit=True) as cur:
      cur.execute(
          "SELECT COALESCE(MAX(user_post_number), 0) + 1 FROM scheduled_posts"
          " WHERE user_id = %s",
          (user_id,),
      )
...
def update_post_time(post_id: int, new_time, recurrence_time=None) -> bool:
  try:
    with db_cursor(commit=True) as cur:
      if recurrence_time:
        cur.execute(
            "UPDATE scheduled_posts SET scheduled_time = %s, recurrence_time ="
            " %s WHERE id = %s",
            (new_time, recurrence_time, post_id),
        )
      else:
        cur.execute(
...
def cancel_post(post_id: int, user_id: int, is_admin: bool = False) -> bool:
  try:
    with db_cursor(commit=True) as cur:
      if is_admin:
        cur.execute(
            "UPDATE scheduled_posts SET status = 'cancelled' WHERE id = %s AND"
            " status = 'pending'",
            (post_id,),
        )
      else:
...
def mark_post_as_deleted(post_id: int):
  try:
    with db_cursor(commit=True) as cur:
      cur.execute(
          "UPDATE scheduled_posts SET status = 'deleted' WHERE id = %s",
          (post_id,),
      )
  except Exception as e:
...
def get_system_stats() -> dict:
  ...
  try:
    with db_cursor() as cur:
      ...
      cur.execute(
          "SELECT COUNT(*) FROM scheduled_posts WHERE status = 'cancelled'"
      )
